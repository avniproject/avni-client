 ...
 */
class ONNXProcessor extends BaseProcessor {
    constructor() {
        super();
    }
    ...
    async runInference(context, modelInput) {
        ...
        try {
            General.logDebug('ONNXProcessor', `Running inference with model: ${modelFile}`);
            ...
            const inferenceResult = await ONNXInferenceModule.runModel(modelFile, modelInput.data, {
                ...
                executionProvider: processorConfig.executionProvider || 'cpu', // cpu, gpu, nnapi
                graphOptimizationLevel: processorConfig.graphOptimizationLevel || 'all',
                intraOpNumThreads: processorConfig.intraOpNumThreads || 1,
                interOpNumThreads: processorConfig.interOpNumThreads || 1
            });
            ...
            return {
                ...
                metadata: {
                    ...
                    inputShape: modelInput.shape,
                    outputType,
                    executionProvider: processorConfig.executionProvider || 'cpu',
                    graphOptimizationLevel: processorConfig.graphOptimizationLevel || 'all',
     ...
     * @param {Object} rawOutput - Raw output from ONNX module
    ...
    async getModelInfo(modelFile) {
        try {
            ...
            return {
                ...
                producerName: modelInfo.producerName,
                graphName: modelInfo.graphName
            };
            
        } catch (error) {
            General.logWarn('ONNXProcessor', `Failed to get model info for ${modelFile}: ${error.message}`);
            return {
                fileName: modelFile,
                error: error.message
            };
    ...
    getCapabilities() {
        return {
            supportedExecutionProviders: ['cpu', 'gpu', 'nnapi'],
            ...
            supportsGraphOptimization: true
        };
    }
    ...
    estimateMemoryUsage(processorConfig) {
        ...
        const modelMemory = inputMemory * 2.5; // ONNX models are typically larger
