...
class SubjectRegistrationState extends AbstractDataEntryState {
    ...
    async validateEntityAgainstRuleAsync(ruleService) {
        return await ruleService.validateAgainstRuleAsync(this.subject, this.formElementGroup.form, "Individual", this.getEntityContext());
    }

    async executeRule(ruleService, context) {
        let decisions = await ruleService.getDecisions(this.subject, "Individual", {}, this.getEntityContext());
