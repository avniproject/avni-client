...
class StartProgramView extends AbstractComponent {
    ...
    proceed(encounter) {
        const selectedEncounter = _.has(encounter, 'operationalEncounterTypeName') ? this.encounterFromEncounterType(encounter) : encounter;
            ...
            <TouchableNativeFeedback onPress={() => {
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.textContainer}>
                        <Text
                            style={[Fonts.typography("paperFontSubhead"), styles.encounterStyle]}>{encounterName}</Text>
                        {!_.isEmpty(displayDate) ?
                            <Text
                                style={[Fonts.typography("paperFontSubhead"), styles.dateStyle]}>{displayDate}</Text> :
    ...
    render() {
        ...
        const types = !this.state.hideUnplanned ? (!_.isEmpty(this.state.encounters) ? [this.I18n.t("plannedVisits"), this.I18n.t("unplannedVisits")] : [this.I18n.t("unplannedVisits")]) : [this.I18n.t("plannedVisits")];
        const visits = !this.state.hideUnplanned ? (!_.isEmpty(this.state.encounters) ? [this.state.encounters, this.state.encounterTypes] : [this.state.encounterTypes]) : [this.state.encounters];
