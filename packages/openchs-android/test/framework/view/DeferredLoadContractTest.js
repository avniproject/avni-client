...
describe("deferred-load contract", () => {
    ...
    // Lost once already, in the 17.3 -> 18.0 merge, and nothing failed when it went.
    //
    // super.isDataLoaded() flips as soon as loadData() (ON_LANDING) returns, but onLanding() zeroes the
    // visits, summary and buttons — the slice is shared across subjects, so without that clear the next
    // subject renders the previous one's data. So the base gate alone lets this tab paint a fully built
    // but EMPTY Programs section from ON_LANDING until ON_LOAD lands (~2.5s on the JSCS database), then
    // pop, running every privilege query twice on the way. state.loaded is what ON_LOAD sets.
    //
    // _loadFailed is the escape hatch: safeDispatch() swallows a throwing ON_LOAD, so without it being
    // read here a failed load sticks on the loader forever — the avni-client#1892 symptom.
    it("SubjectDashboardProgramsTab gates its loader on ON_LOAD, with a failure escape", () => {
        const src = read("views/program/SubjectDashboardProgramsTab.js");
        expect(src).toMatch(/\n {4}isDataLoaded\(\) \{/);
        const body = src.split(/\n {4}isDataLoaded\(\) \{/)[1].split("\n    }")[0];
        expect(body).toMatch(/super\.isDataLoaded\(\)/);
        expect(body).toMatch(/this\.state\.loaded/);
        expect(body).toMatch(/this\._loadFailed/);
    });

    // safeDispatch() catches instead of rethrowing, so the base class never sees the error and
    // _loadError stays nil. Something must therefore read _loadFailed, or the catch is a silent no-op.
    it("a screen that catches its own dispatch errors also reads the flag it sets", () => {
        const offenders = allScreenFiles().filter((rel) => {
            const src = read(rel);
            if (!/this\._loadFailed = true/.test(src)) return false;
            return (src.match(/_loadFailed/g) || []).length < 2;
        });
        expect(offenders).toEqual([]);
    });
});
