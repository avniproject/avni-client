...
class TaskService extends BaseService {
    ...
    getFilteredTasks(taskFilter: TaskFilter) {
        ...
        if (!_.isNil(taskFilter.taskScheduledDate))
            {
                let scheduledDateStart = moment(taskFilter.taskScheduledDate).startOf('day');
                tasks = tasks.filtered("scheduledOn >= $0 && scheduledOn < $1", scheduledDateStart.toDate(), scheduledDateStart.add(1, 'days').toDate());
            }
        if (!_.isNil(taskFilter.taskCompletedDate))
            {
                let completedDateStart = moment(taskFilter.taskCompletedDate).startOf('day');
                tasks = tasks.filtered("completedOn >= $0 && completedOn < $1", completedDateStart.toDate(), completedDateStart.add(1, 'days').toDate());
            }
