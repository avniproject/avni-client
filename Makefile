...
define _create_config
	...
	@if [ $(1) = "prod" ]; then \
		echo "import envConfig from '../../config/env/$(1).json';\n\nexport default Object.assign({}, envConfig, {COMMIT_ID: '$(sha)', SERVER_URL: '$(flavor_server_url)', DISABLE_APP_RUN_ON_ROOTED_DEVICES: $(flavor_disable_app_run_on_rooted_devices)});" > packages/openchs-android/src/framework/Config.js; \
	else \
	 	echo "import envConfig from '../../config/env/$(1).json';\n\nexport default Object.assign({}, envConfig, {COMMIT_ID: '$(sha)'});" > packages/openchs-android/src/framework/Config.js; \
	fi
endef
...
_prebuild_disabled: ## Autolinking.json now generated by Gradle task automatically
