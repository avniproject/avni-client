    ...
    saveOrUpdate(encounter, nextScheduledVisits = [], skipCreatingPendingStatus) {
        ...
        this.db.write(() => {
            if (!skipCreatingPendingStatus && isApprovalEnabled)
                encounter.addUpdateApprovalStatus(entityApprovalStatusService.createPendingStatus(encounter.uuid, Encounter.schema.name, db, encounter.encounterType.uuid));
