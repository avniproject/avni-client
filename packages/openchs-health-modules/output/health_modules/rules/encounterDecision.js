...
var defaultWeightRangesToCode = [{ start: 3.0, end: 5.5, code: "X1" }, { start: 5.6, end: 8.0, code: "X2" }, { start: 8.1, end: 13.0, code: "X3" }, { start: 13.1, end: 16.0, code: "X4" }, { start: 16.1, end: 25.5, code: "X5" }, { start: 25.6, end: 32.5, code: "X6" }, { start: 32.6, end: 1000, code: "X7" }];

var weightRangesToCodeForLonartForte = [{ start: 3.0, end: 4.9, code: "X0" }, { start: 5.0, end: 14.9, code: "X1" }, { start: 15, end: 24.9, code: "X2" }, { start: 25, end: 34.9, code: "X3" }, { start: 35, end: 1000, code: "X4" }];
...
var doseQuantityToMarathi = function doseQuantityToMarathi(doseQuantity, doseUnit) {
...
var dosageTimingToMarathi = function dosageTimingToMarathi(complaint, times) {
...
var getDoseUnitMessage = function getDoseUnitMessage(daysPrescription) {
    if (daysPrescription["Dose Unit"] === "Spoon") return daysPrescription.Amount < 2 ? "चमचा" : "चमचे";else if (daysPrescription["Dose Unit"] === "ml") return "ml";
    return "टॅबलेट";
};
...
var getWeightRangeToCode = function getWeightRangeToCode(complaint, weight) {
    var weightRangeToCodeMap = complaintToWeightRangesToCodeMap[complaint];
    if (weightRangeToCodeMap === undefined || weightRangeToCodeMap === null) weightRangeToCodeMap = defaultWeightRangesToCode;
...
var hasMalaria = function hasMalaria(paracheckResult) {
    return paracheckResult !== undefined && paracheckResult.length === 1 && paracheckResult[0].includes("Positive");
};

var getDecisions = function getDecisions(encounter) {
    ...
    for (var complaintIndex = 0; complaintIndex < params.complaints.length; complaintIndex++) {
        ...
        for (var token = 0; token < dayTokens.length; token++) {
            ...
            for (var medicineIndex = 0; medicineIndex < prescription[dayTokens[token]].length; medicineIndex++) {
                ...
                if (dayTokens.length === 1 && firstToken !== "1") {
                    if (firstToken === "3-5") message += "३ किंवा ५ दिवसांसाठी";else if (firstToken === "1-5") message += "१ ते ५ दिवस";else if (firstToken === "3") message += "३ दिवस";else if (firstToken === "2") message += "२ दिवस";
        ...
        if (params.weight >= 13 && params.complaints[complaintIndex] === 'Malaria') decision.alert = "क्लोरोक्विन व पॅरासिटामॉल ही औषधे जेवल्यावर खायला सांगावी";else if (params.complaints[complaintIndex] === 'Vomiting') decision.alert = "उलटी असल्यास आधी औषध द्यावे व अर्ध्या तासांनंतर जेवण, दुध द्यावे व अर्ध्या तासांनंतर इतर औषधे द्यावीत";else if (params.complaints[complaintIndex] === 'Chloroquine Resistant Malaria' && (params.age >= 16 || params.age <= 40) && params.sex === "Female") {
...
var validate = function validate(encounter, form) {
    if (encounter.encounterType.name !== "Outpatient") return [];
    ...
    for (var complaintIndex = 0; complaintIndex < params.complaints.length; complaintIndex++) {
        var complaint = params.complaints[complaintIndex];
        var weightRangeToCode = getWeightRangeToCode(complaint, params.weight);
