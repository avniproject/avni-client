...
describe('ApprovalAnswersSave', () => {
    let service, writes, savedStatuses, queued, persisted, storedRows;
    ...
    beforeEach(() => {
        ...
        const db = {
            ...
            objects: () => ({filtered: () => []}),
            objectForPrimaryKey: (schemaName, key) => storedRows[key]
        };

        // Services write through the repository layer, so each repository is a thin pass-through to the
        // recording db above and the transaction manager is its write.
        const repositoryFactory = {
            getRepository: (schemaName) => ({
                create: (entity, updateMode) => db.create(schemaName, entity, updateMode),
                objectForPrimaryKey: (key) => db.objectForPrimaryKey(schemaName, key),
                persistMutations: (entity) => persisted.push(entity)
            }),
            transactionManager: {write: (fn) => db.write(fn)}
        };
        service = new EntityApprovalStatusService(db, {getRepositoryFactory: () => repositoryFactory});
        service.db = db;
    ...
    it('leaves the pending status created on save untouched', () => {
        ...
        service.createPendingStatus(subject, Individual.schema.name, 'subject-type-uuid');
    ...
    /**
     * A correction assigns the answers onto the stored row. On Realm that assignment is the write; SQLite
     * hands back a plain copy, so without an explicit re-persist the correction would be queued for sync
     * but never reach the local database.
     */
    describe('correcting the answers on a recorded decision', () => {
        it('persists the new answers onto the same decision and queues it, leaving the status alone', () => {
            const stored = {uuid: 'decision-1', approvalStatus: {status: ApprovalStatus.statuses.Approved}, observations: []};
            storedRows['decision-1'] = stored;

            service.updateDecisionAnswers({uuid: 'decision-1'}, [anAnswer('concept-1', 'Corrected')]);

            assert.equal(1, writes);
            assert.deepEqual([stored], persisted, 'the mutated row must be re-persisted for SQLite');
            assert.equal('concept-1', stored.observations[0].concept.uuid);
            assert.equal(ApprovalStatus.statuses.Approved, stored.approvalStatus.status);
            assert.equal(0, savedStatuses.length, 'a correction must not write a second decision');
            assert.equal(1, queued.length);
        });

        it('refuses to write when the decision is not stored', () => {
            assert.throws(() => service.updateDecisionAnswers({uuid: 'missing'}, []), /No approval decision missing/);
            assert.equal(0, persisted.length);
            assert.equal(0, queued.length);
        });
    });
});
