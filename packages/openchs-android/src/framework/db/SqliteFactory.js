...
class SqliteFactory {
    static async createSqliteDb() {
        ...
        if (SpikeFlags.SYNC_NORMAL) db.executeSync("PRAGMA synchronous = NORMAL");
        SqliteFactory._spikeProbe(db, !_.isNil(encryptionKey));
    ...
    // SPIKE: log the effective PRAGMAs once per open.
    static _spikeProbe(db, encrypted) {
        try {
            const one = (sql) => {
            ...
            spikeLog("db_open", {
                synchronous: one("PRAGMA synchronous"), journal: one("PRAGMA journal_mode"),
                pageSize: one("PRAGMA page_size"), cacheSize: one("PRAGMA cache_size"),
                sqlite: one("SELECT sqlite_version()"), cipher: one("PRAGMA cipher_version"), encrypted
            });
        } catch (e) {
            spikeLog("db_open_probe_error", {message: e.message});
        }
    }
