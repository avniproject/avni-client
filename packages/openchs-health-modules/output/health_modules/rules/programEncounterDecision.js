...
function C() {
    ...
    this.getMatchingKey = function (obsValue, masterData) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (nextKeyDifference < currentKeyDifference) continue;else return currentLength;
        }
    };
...
var getDecisions = function getDecisions(programEncounter, today) {
...
var programDecision = __webpack_require__(4);
var observationConditions = __webpack_require__(5).observationRules;
...
module.exports.getDecisions = function (programEncounter, today) {
    ...
    if (programEncounter.encounterType.name === 'ANC') {
        var addComplicationsObservation = function addComplicationsObservation() {
            decisions.push({ name: 'High Risk Conditions', value: [] });
        };

        var analyseOtherRisks = function analyseOtherRisks() {
            var weight = getObservationValue('Weight');
            if (!C.isNil(weight) && weight <= 35) addComplication('Underweight');
        };

        var addComplication = function addComplication(conceptName) {
        ...
        var getObservationValueFromEntireEnrolment = function getObservationValueFromEntireEnrolment(conceptName) {
            return programEncounter.programEnrolment.getObservationValueFromEntireEnrolment(conceptName, programEncounter);
        };

        var getObservationValue = function getObservationValue(conceptName) {
            return programEncounter.getObservationValue(conceptName);
        };

        var observationExistsInEntireEnrolment = function observationExistsInEntireEnrolment(conceptName) {
            return programEncounter.programEnrolment.getObservationValueFromEntireEnrolment(conceptName, programEncounter);
        };

        var analyseHypertensiveRisks = function analyseHypertensiveRisks() {
            var systolic = getObservationValue('Systolic');
            var diastolic = getObservationValue('Diastolic');
            var urineAlbumin = getObservationValue('Urine Albumin');
            var obsHistory = getObservationValueFromEntireEnrolment('Obstetrics History');

            var mildPreEclempsiaUrineAlbuminValues = ['Trace', '+1', '+2'];
            var severePreEclempsiaUrineAlbuminValues = ['+3', '+4'];

            if (urineAlbumin === undefined) investigationAdviceBuilder.add("Urine Albumin Test");

            var isBloodPressureHigh = systolic >= 140 || diastolic >= 90; //can go in high risk category
            var urineAlbuminIsMild = C.contains(mildPreEclempsiaUrineAlbuminValues, urineAlbumin);
            var urineAlbuminIsSevere = C.contains(severePreEclempsiaUrineAlbuminValues, urineAlbumin);
            var obsHistoryOfPregnancyInducedHypertension = C.contains(obsHistory, 'Pregnancy Induced Hypertension');
            var hasConvulsions = getObservationValue('Convulsions') === "Present"; //will be identified in hospital
            var highRiskConditions = getObservationValueFromEntireEnrolment('High Risk Conditions');
            var isChronicHypertensive = highRiskConditions && highRiskConditions.indexOf('Chronic Hypertension') >= 0;
            ...
            if (pregnancyPeriodInWeeks <= 20 && isBloodPressureHigh) {
                ...
                if (!obsHistoryOfPregnancyInducedHypertension && isBloodPressureHigh) {
                    ...
                    if (hasConvulsions && (urineAlbuminIsMild || urineAlbuminIsSevere)) addComplication('Eclampsia');else if (!hasConvulsions && urineAlbuminIsMild) addComplication('Mild Pre-Eclampsia');else if (!hasConvulsions && urineAlbuminIsSevere) addComplication('Severe Pre-Eclampsia');
                }
            }
        };

        var analyseAnemia = function analyseAnemia() {
            //anm also does this test
            ...
            } else if (hemoglobin > 11) decisions.push({
                name: 'Treatment Advice',
                value: "Hb normal. Proceed with Prophylactic treatment against anaemia"
            });
        };

        var manageVaginalBleeding = function manageVaginalBleeding() {
            var pregnancyComplaints = getObservationValueFromEntireEnrolment("Pregnancy Complaints");
            var vaginalBleeding = pregnancyComplaints && pregnancyComplaints.indexOf("PV bleeding") >= 0;
        ...
        var analyseSexuallyTransmittedDisease = function analyseSexuallyTransmittedDisease() {
        ...
        var analyseSickling = function analyseSickling() {
        ...
        var analyseHepatitisB = function analyseHepatitisB() {
            var hepatitisB = getObservationValueFromEntireEnrolment('HbsAg');
            if (hepatitisB === 'Positive') addComplication('Hepatitis B Positive');
        };

        var analyseMalaria = function analyseMalaria() {
            var paracheck = getObservationValueFromEntireEnrolment('Paracheck');
            if (paracheck === 'Positive for PF' || paracheck === 'Positive for PV' || paracheck === 'Positive for PF and PV') addComplication('Malaria');
        };

        var analyseFoetalPresentation = function analyseFoetalPresentation() {
        ...
        var lmpDate = programEncounter.programEnrolment.getObservationValue('Last Menstrual Period');
        var pregnancyPeriodInWeeks = C.getWeeks(lmpDate, programEncounter.encounterDateTime);

        var investigationAdviceBuilder = new InvestigationAdviceBuilder();
...
module.exports = {};
module.exports.getNextScheduledVisits = __webpack_require__(6).getNextScheduledVisits;
...
module.exports.getDecisions = function (programEnrolment, today, programEncounter) {
    ...
    function addIfNotExists(conceptName) {
        ...
        if (!observationExistsInEntireEnrolment(conceptName)) highRiskConditions.push(conceptName);
    }
    ...
    function analyseOtherRisks() {
        var height = getObservationValueFromEntireEnrolment('Height');
        if (height !== undefined && height <= 145) addIfNotExists('Short Stature');

        if (programEnrolment.individual.getAgeInYears(today) > 30) addIfNotExists('Old age pregnancy');

        if (programEnrolment.individual.getAgeInYears(today) < 18) addIfNotExists('Under age pregnancy');

        if (programEnrolment.getObservationValue('Number of abortion') > 0) addIfNotExists('Previous Abortion(s)');

        if (programEnrolment.getObservationValue('Gravida') >= 5) addIfNotExists('Grand Multipara');
    }

    function analyzeHistoryBasedComplications() {
        var obstetricsHistory = getObservationValueFromEntireEnrolment('Obstetrics History');
...
var getNextScheduledVisits = function getNextScheduledVisits(programEnrolment, today, currentEncounter) {
    var lmpConceptName = 'Last Menstrual Period';

    var encounters = [];

    var lmpDate = programEnrolment.getObservationValue(lmpConceptName);

    var matchingEncounter = function matchingEncounter(encounters, encounterTypeName, encounterName) {
        return encounters.find(function (encounter) {
            return encounter.encounterType.name === encounterTypeName && encounter.name === encounterName;
        });
    };

    var currentEncounters = programEnrolment.encounters;

    var deliveryEncounter = matchingEncounter(currentEncounters, 'Delivery', 'Delivery');

    var deliveryDate = deliveryEncounter && deliveryEncounter.encounterDateTime;
    console.log("Delivery encounter is " + deliveryEncounter);
    console.log("Delivery date is " + deliveryDate);

    var addEncounter = function addEncounter(baseDate, encounterType, name) {
...
var allImports = {};
allImports.Mother = __webpack_require__(3);
allImports.Child = __webpack_require__(2);
...
var executeProgramEncounterFunc = function executeProgramEncounterFunc(config) {
    var today = new Date();
    var enrolment = config.parameter.programEnrolment;
    var programName = enrolment.program.name;
    var fn = targetFunction(config, programName);
